...
namespace vili::writer
{
    ...
    std::string indent_except_last_delimiter(const std::string& value,
        unsigned int indent_level, const bool leading_indent = true)
    {
        const auto last_newline = value.rfind('\n');
        if (last_newline != std::string::npos)
        {
            const std::string after_last_newline
                = std::string(value.begin() + last_newline + 1, value.end());
            if (after_last_newline.empty())
            {
                return indent(value, indent_level, leading_indent);
            }
            const char delimiter = after_last_newline.at(after_last_newline.size() - 1);
            if (std::count(after_last_newline.begin(), after_last_newline.end(), ' ')
                        + std::count(
                            after_last_newline.begin(), after_last_newline.end(), '\t')
                    == after_last_newline.size() - 1
                && (delimiter == '}' || delimiter == ']'))
            {
                std::string indentable_part
                    = std::string(value.begin(), value.begin() + last_newline);
                std::string remainder
                    = std::string(value.begin() + last_newline, value.end());
                if (indentable_part.at(indentable_part.size() - 1) == '\n')
                {
                    indentable_part
                        = std::string(value.begin(), value.begin() + last_newline - 1);
                    remainder
                        = std::string(value.begin() + last_newline - 1, value.end());
                }
                const std::string result
                    = indent(indentable_part, indent_level, leading_indent) + remainder;
                return result;
            }
        }
        return indent(value, indent_level, leading_indent);
    ...
    std::string dump_array(const vili::node& data, const dump_options& options)
    {
        ...
        new_options.object.style = object_style::braces;
        ...
        {
            const std::string item_dump = dump(item, new_options);
        ...
        for (auto it = values_dumps.begin(); it != values_dumps.end(); ++it)
        {
            ...
            if (must_indent)
            {
                current_value_dump = indent_except_last_delimiter(
                    current_value_dump, options.indent, false);
            }
            must_indent = false;
        ...
        {
            current_line
                = indent_except_last_delimiter(current_line, options.indent) + "\n";
        }
        dump_value += current_line;
    ...
    std::string dump_object(const vili::node& data, const dump_options& options)
    {
        ...
        for (const auto& [key, value] : data.items())
        {
            ...
            if (!value.is_null())
            {
                ...
                const std::string current_dump = values_dumps[key];
                current_line
                    += indent_except_last_delimiter(current_dump, options.indent, false);
            }
